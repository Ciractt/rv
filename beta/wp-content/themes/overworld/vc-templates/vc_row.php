...
if ( $row_content_width !== 'grid' ) {
	...
	if ( ! empty( $simple_background_image ) ) {
		...
		$edgtf_vc_row_style[] = 'background-image: url(' . esc_url( is_array( $image_src ) ? $image_src[0] : $image_src ) . ')';
	}
	...
	if ( ! empty( $parallax_background_image ) ) {
		...
		$wrapper_attributes[] = 'data-parallax-bg-image="' . esc_url( is_array( $image_src ) ? $image_src[0] : $image_src ) . '"';
