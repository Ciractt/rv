...
class EdgeSkin extends OverworldEdgeClassSkinAbstract {
	
    /**
     * Skin constructor. Hooks to overworld_edge_admin_scripts_init and overworld_edge_enqueue_admin_styles
     */
	public function __construct() {
		$this->skinName = 'edge';
		
		// Admin register scripts and styles
		add_action( 'overworld_edge_action_admin_scripts_init', array( $this, 'registerStyles' ) );
		add_action( 'overworld_edge_action_admin_scripts_init', array( $this, 'registerScripts' ) );
		
		// Admin enqueue scripts and styles
		add_action( 'overworld_edge_action_enqueue_admin_styles', array( $this, 'enqueueStyles' ) );
		add_action( 'overworld_edge_action_enqueue_admin_scripts', array( $this, 'enqueueScripts' ) );
		
		// Admin enqueue meta box scripts and styles
		add_action( 'overworld_edge_action_enqueue_meta_box_styles', array( $this, 'enqueueStyles' ) );
		add_action( 'overworld_edge_action_enqueue_meta_box_scripts', array( $this, 'enqueueScripts' ) );
	...
	public function registerScripts() {
		...
		foreach ( $this->scripts as $scriptHandle => $script ) {
			overworld_edge_register_skin_script( $scriptHandle, $script['path'], $script['dependency'] );
		}
	}
    ...
    public function registerStyles() {
        ...
        foreach ($this->styles as $styleHandle => $stylePath) {
            overworld_edge_register_skin_style($styleHandle, $stylePath);
        }
    }
    ...
    public function renderOptions() {
        ...
        $tab    = overworld_edge_get_admin_tab();
        $active_page = $overworld_edge_global_Framework->edgtOptions->getAdminPageFromSlug($tab);
     ...
     *
     * @see OverworldEdgeClassSkinAbstract::loadTemplatePart()
     */
     ...
     *
     * @param OverworldEdgeClassAdminPage $page current page to load
     * @see OverworldEdgeClassSkinAbstract::loadTemplatePart()
     */
