(function($){
		...
		wp.media.customlibEditGallery1 = {
			frame: function () {
				...
				this._frame.on('update', function () {
					...
					var data = {
						action: 'overworld_edge_gallery_upload_get_images',
						ids: ids,
	...
	function edgtfInitAjaxForm() {
		$('#edgtf_top_save_button').on('click', function () {
			$('.edgtf_ajax_form').trigger('submit');
		...
		$(document).on("submit", ".edgtf_ajax_form", function (a) {
			var b = $(this),
				c = {
					action: "overworld_edge_save_options"
				};
		...
		if (holder.length) {
			...
			edgtfImportBtn.on('click', function (e) {
				...
				if (confirm('Are you sure, you want to import Options now?')) {
					...
					var data = {
						action: 'overworld_core_import_theme_options',
						content: importValue,
						nonce: importNonce
					};
		...
		if (holder.length) {
			...
			edgtfImportBtn.on('click', function (e) {
				...
				if (confirm('Are you sure, you want to import Custom Sidebars now?')) {
					...
					var data = {
						action: 'overworld_core_import_custom_sidebars',
						content: importValue,
						nonce: importNonce
					};
		...
		if (holder.length) {
			...
			edgtfImportBtn.on('click', function (e) {
				...
				if (confirm('Are you sure, you want to import Widgets now?')) {
					...
					var data = {
						action: 'overworld_core_import_widgets',
						content: importValue,
						nonce: importNonce
					};
	...
	function edgtfInitImportContent() {
		...
		if (edgtfImportHolder.length) {
			...
			var edgtfInitContentImport = function(import_expl, progress_bar, progress_value, import_only_content) {
				var xml_file_name = index < 10 ? 'overworld_content_0' + index + '.xml' : 'overworld_content_' + index + '.xml',
				...
				jQuery.ajax({
					...
					url: ajaxurl,
					data: {
						action: 'overworld_core_action_import_content',
						xml: xml_file_name,
						example: import_expl,
						...
						if (index < 10) {
							...
							if (import_only_content === true) {
								...
								jQuery.ajax({
									...
									url: ajaxurl,
									data: {
										action: 'overworld_core_action_import_other_elements',
										example: import_expl
									},
									success: function (data, textStatus, XMLHttpRequest) {
										edgtfSetCompletedMessage(progress_value, progress_bar);
				...
				if (confirm(confirmMessage)) {
					...
					if (import_opt === 'content') {
						...
						jQuery.ajax({
							...
							url: ajaxurl,
							data: {
								action: 'overworld_core_action_import_widgets',
								example: import_expl
							},
							success: function (data, textStatus, XMLHttpRequest) {
								edgtfSetCompletedMessage(progress_value, progress_bar);
						...
						jQuery.ajax({
							...
							url: ajaxurl,
							data: {
								action: 'overworld_core_action_import_options',
								example: import_expl
							},
							success: function (data, textStatus, XMLHttpRequest) {
								edgtfSetCompletedMessage(progress_value, progress_bar);
