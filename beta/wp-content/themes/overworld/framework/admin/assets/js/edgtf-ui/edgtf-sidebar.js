(function ( $ ) {
	...
	EdgefSidebar.prototype = {
		...
		add_del_button: function () {
			var wrapper = this.widget_wrap.find( '[data-widget-area-id*="edgtf-custom-sidebar-"]' );
			if ( wrapper.length ) {
				wrapper.parents( '.wp-block-widget-area' ).parent().append( '<span class="edgtf-delete-button"></span>' );
			}
			var $gutenbergWidgetsArea = $( '.block-editor-writing-flow' );
			if ( $gutenbergWidgetsArea.length ) {
				var customSidebars = typeof edge === 'object' && typeof edge.customSidebars !== 'undefined' ? edge.customSidebars : [];
						...
						setTimeout(
							function () {
								...
								if ( customWidgetArea.length ) {
									customWidgetArea.parents( '.components-panel__body' ).children( '.components-panel__body-title' ).append( '<span class="edgtf-delete-button"></span>' );
								}
							},
							3000
						);
						...
						widget.slideUp(
							...
							function () {
								...
								$(
									...
									widget
								).trigger( 'click' ); //delete all widgets inside
								widget.remove();
								wpWidgets.saveOrder();
							}
						);
