...
include OVERWORLD_EDGE_FRAMEWORK_ICONS_ROOT_DIR . '/icons-pack-interface.php';

foreach ( glob( OVERWORLD_EDGE_FRAMEWORK_ICONS_ROOT_DIR . '/*/load.php' ) as $module_load ) {
	if ( overworld_edge_is_customizer_item_enabled( $module_load, 'overworld_performance_disable_icon_pack_' ) ) {
		include_once $module_load;
	}
}

if ( ! function_exists( 'overworld_edge_get_icon_pack_collections' ) ) {
	function overworld_edge_get_icon_pack_collections() {
		$options = apply_filters( 'overworld_edge_filter_add_icon_pack_into_collection', $options = array() );
		
		return $options;
	}
}

if ( ! function_exists( 'overworld_edge_get_icon_pack_options' ) ) {
	function overworld_edge_get_icon_pack_options() {
		$options = apply_filters( 'overworld_edge_filter_add_icon_pack_into_options', $options = array() );
		
		return $options;
	}
}

if ( ! function_exists( 'overworld_edge_get_icon_pack_social_options' ) ) {
	function overworld_edge_get_icon_pack_social_options() {
		$options = apply_filters( 'overworld_edge_filter_add_icon_pack_into_social_options', $options = array() );
...
/*
 * Class: OverworldEdgeClassIconCollections
 */
class OverworldEdgeClassIconCollections {
	...
	private function __construct() {
		...
		$this->iconPackCollections   = overworld_edge_get_icon_pack_collections();
		$this->iconPackOptions       = overworld_edge_get_icon_pack_options();
		$this->iconPackSocialOptions = overworld_edge_get_icon_pack_social_options();
	...
	public function getIconsMetaBoxOrOption( $attributes ) {
		...
		foreach ( $icon_collections as $collection_key => $collection_object ) {
			...
			$icon_pack_container = overworld_edge_add_admin_container(
				array(
					'parent'     => $parent,
	...
	public function getVCParamsArray( $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false ) {
		...
		$iconPackParams = array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Icon Pack', 'overworld' ),
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'        => 'dropdown',
					'heading'     => esc_html__( 'Icon', 'overworld' ),
	...
	public function getIconWidgetParamsArray() {
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'name'    => 'icon_pack',
			'title'   => esc_html__( 'Icon Pack', 'overworld' ),
			'options' => $this->getIconPackOptions()
		);
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'    => 'dropdown',
					'title'   => $collection->title . esc_html__( ' Icon', 'overworld' ),
	...
	public function getSocialIconWidgetMultipleParamsArray( $count ) {
		...
		for ( $n = 1; $n <= $count; $n ++ ) {
			if ( is_array( $iconCollectionsVC ) && count( $iconCollectionsVC ) ) {
				foreach ( $iconCollectionsVC as $key => $collection ) {
					$iconOps[] = array(
						...
						'title'   => sprintf( esc_html__( 'Icon %s %s Icon', 'overworld' ), $n, $collection->title ),
			...
			$iconOps[] = array(
				'type'  => 'textfield',
				'name'  => 'link_' . $n,
				'title' => sprintf( esc_html__( 'Link %s', 'overworld' ), $n )
			);
			
			$iconOps[] = array(
				'type'    => 'dropdown',
				'name'    => 'target_' . $n,
				'title'   => sprintf( esc_html__( 'Link Target %s', 'overworld' ), $n ),
	...
	public function getIconSizesArray() {
		return array(
			esc_html__( 'Tiny', 'overworld' )       => 'fa-lg',
			esc_html__( 'Small', 'overworld' )      => 'fa-2x',
			esc_html__( 'Medium', 'overworld' )     => 'fa-3x',
			esc_html__( 'Large', 'overworld' )      => 'fa-4x',
			esc_html__( 'Very Large', 'overworld' ) => 'fa-5x'
		);
	}
	...
	public function getIconCollectionIcons( iOverworldEdgeInterfaceIconCollection $collection ) {
	...
	public function getIconCollectionsEmpty( $no_empty_key = "" ) {
		$array                  = array();
		$array[ $no_empty_key ] = esc_html__( 'No Icon', 'overworld' );
	...
	public function getIconCollectionsVCEmpty() {
		$vc_array                                           = array();
		$vc_array[ esc_html__( 'No Icon', 'overworld' ) ] = '';
	...
	public function enqueueStyles() {
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $collection_key => $collection_obj ) {
				wp_enqueue_style( 'overworld-edge-' . $collection_key, $collection_obj->styleUrl );
			}
		}
	}
