...
if ( ! function_exists( 'overworld_edge_header_skin_class' ) ) {
	/**
	 * Function that adds header style class to body tag
	 */
	function overworld_edge_header_skin_class( $classes ) {
		$header_style     = overworld_edge_get_meta_field_intersect( 'header_style', overworld_edge_get_page_id() );
		$header_style_404 = overworld_edge_options()->getOptionValue( '404_header_style' );
	...
	add_filter( 'body_class', 'overworld_edge_header_skin_class' );
}

if ( ! function_exists( 'overworld_edge_sticky_header_behaviour_class' ) ) {
	/**
	 * Function that adds header behavior class to body tag
	 */
	function overworld_edge_sticky_header_behaviour_class( $classes ) {
		$header_behavior = overworld_edge_get_meta_field_intersect( 'header_behaviour', overworld_edge_get_page_id() );
	...
	add_filter( 'body_class', 'overworld_edge_sticky_header_behaviour_class' );
}

if ( ! function_exists( 'overworld_edge_menu_dropdown_appearance' ) ) {
	/**
	 ...
	 */
	function overworld_edge_menu_dropdown_appearance( $classes ) {
		$dropdown_menu_appearance = overworld_edge_options()->getOptionValue( 'menu_dropdown_appearance' );
	...
	add_filter( 'body_class', 'overworld_edge_menu_dropdown_appearance' );
}

if ( ! function_exists( 'overworld_edge_header_class' ) ) {
	/**
	 ...
	 */
	function overworld_edge_header_class( $classes ) {
		...
		$disable_menu_area_shadow = overworld_edge_get_meta_field_intersect( 'menu_area_shadow', $id ) == 'no';
		if ( $disable_menu_area_shadow ) {
			$classes[] = 'edgtf-menu-area-shadow-disable';
		}
		
		$disable_menu_area_grid_shadow = overworld_edge_get_meta_field_intersect( 'menu_area_in_grid_shadow', $id ) == 'no';
		if ( $disable_menu_area_grid_shadow ) {
			$classes[] = 'edgtf-menu-area-in-grid-shadow-disable';
		}
		
		$disable_menu_area_border = overworld_edge_get_meta_field_intersect( 'menu_area_border', $id ) == 'no';
		if ( $disable_menu_area_border ) {
			$classes[] = 'edgtf-menu-area-border-disable';
		}
		
		$disable_menu_area_grid_border = overworld_edge_get_meta_field_intersect( 'menu_area_in_grid_border', $id ) == 'no';
		if ( $disable_menu_area_grid_border ) {
			$classes[] = 'edgtf-menu-area-in-grid-border-disable';
		}
		
		if ( overworld_edge_get_meta_field_intersect( 'menu_area_in_grid', $id ) == 'yes' &&
		     overworld_edge_get_meta_field_intersect( 'menu_area_grid_background_color', $id ) !== '' &&
		     overworld_edge_get_meta_field_intersect( 'menu_area_grid_background_transparency', $id ) !== '0'
		) {
			$classes[] = 'edgtf-header-menu-area-in-grid-padding';
		}
		
		$disable_logo_area_border = overworld_edge_get_meta_field_intersect( 'logo_area_border', $id ) == 'no';
		if ( $disable_logo_area_border ) {
			$classes[] = 'edgtf-logo-area-border-disable';
		}
		
		$disable_logo_area_grid_border = overworld_edge_get_meta_field_intersect( 'logo_area_in_grid_border', $id ) == 'no';
		if ( $disable_logo_area_grid_border ) {
			$classes[] = 'edgtf-logo-area-in-grid-border-disable';
		}
		
		if ( overworld_edge_get_meta_field_intersect( 'logo_area_in_grid', $id ) == 'yes' &&
		     overworld_edge_get_meta_field_intersect( 'logo_area_grid_background_color', $id ) !== '' &&
		     overworld_edge_get_meta_field_intersect( 'logo_area_grid_background_transparency', $id ) !== '0'
		) {
			$classes[] = 'edgtf-header-logo-area-in-grid-padding';
		}
		
		$disable_shadow_vertical = overworld_edge_get_meta_field_intersect( 'vertical_header_shadow', $id ) == 'no';
		if ( $disable_shadow_vertical ) {
			$classes[] = 'edgtf-header-vertical-shadow-disable';
		}
		
		$disable_border_vertical = overworld_edge_get_meta_field_intersect( 'vertical_header_border', $id ) == 'no';
	...
	add_filter( 'body_class', 'overworld_edge_header_class' );
}

if ( ! function_exists( 'overworld_edge_header_area_style' ) ) {
	/**
	 * Function that return styles for header area
	 */
	function overworld_edge_header_area_style( $style ) {
		...
		$menu_area_background_color_rgba = overworld_edge_rgba_color( $menu_area_background_color, $menu_area_background_transparency );
		...
		if ( $menu_area_height !== '' ) {
			$menu_area_style['height'] = overworld_edge_filter_px( $menu_area_height ) . 'px !important';
		}
		...
		if ( $container_side_padding !== '' ) {
			...
			$current_style .= overworld_edge_dynamic_css( $menu_container_selector, $menu_container_styles );
		}
		...
		$menu_area_grid_background_color_rgba = overworld_edge_rgba_color( $menu_area_grid_background_color, $menu_area_grid_background_transparency );
		
		if ( ! empty( $menu_area_grid_background_color_rgba ) ) {
			$menu_area_grid_style['background-color'] = $menu_area_grid_background_color_rgba;
		}
		
		$current_style .= overworld_edge_dynamic_css( $menu_area_selector, $menu_area_style );
		$current_style .= overworld_edge_dynamic_css( $menu_area_grid_selector, $menu_area_grid_style );
		...
		if ( $dropdown_top_position !== '' ) {
			$dropdown_styles['top'] = overworld_edge_filter_suffix( $dropdown_top_position, '%' ) . '%';
		}
		
		$dropdown_selector = array( $class_prefix . ' .edgtf-page-header .edgtf-drop-down .second' );
		
		$current_style .= overworld_edge_dynamic_css( $dropdown_selector, $dropdown_styles );
		...
		$logo_area_background_color_rgba = overworld_edge_rgba_color( $logo_area_background_color, $logo_area_background_transparency );
		...
		if ( $logo_area_height !== '' ) {
			$logo_area_style['height'] = overworld_edge_filter_px( $logo_area_height ) . 'px !important';
		}
		...
		$logo_area_grid_background_color_rgba = overworld_edge_rgba_color( $logo_area_grid_background_color, $logo_area_grid_background_transparency );
		...
		if ( ! empty( $logo_area_style ) ) {
			$current_style .= overworld_edge_dynamic_css( $logo_area_selector, $logo_area_style );
		}
		
		if ( ! empty( $logo_area_grid_style ) ) {
			$current_style .= overworld_edge_dynamic_css( $logo_area_grid_selector, $logo_area_grid_style );
		}
		
		$current_style = apply_filters( 'overworld_edge_filter_add_header_page_custom_style', $current_style, $class_prefix, $page_id ) . $style;
		
		return $current_style;
	}
	
	add_filter( 'overworld_edge_filter_add_page_custom_style', 'overworld_edge_header_area_style' );
}


if ( ! function_exists( 'overworld_edge_logo_holder_style' ) ) {
	/**
	 * Function that return styles for header logo holder
	 */
	function overworld_edge_logo_holder_style( $style ) {
		$page_id              = overworld_edge_get_page_id();
		$class_prefix         = overworld_edge_get_unique_page_class( $page_id, true );
		$logo_holder_selector = array( $class_prefix . ' .edgtf-page-header .edgtf-menu-area .edgtf-logo-wrapper a' );

		$current_style = '';

		$logo_holder_style     = array();
		$logo_overlapping      = overworld_edge_get_meta_field_intersect( 'logo_allow_outside_header', $page_id );

		if($logo_overlapping === 'yes') {
			$logo_holder_style['max-height'] = 'none !important';
		}

		if ( ! empty( $logo_holder_style ) ) {
			$current_style .= overworld_edge_dynamic_css( $logo_holder_selector, $logo_holder_style );
		}
	...
	add_filter( 'overworld_edge_filter_add_page_custom_style', 'overworld_edge_logo_holder_style' );
}

if ( ! function_exists( 'overworld_edge_logo_image_style' ) ) {
	/**
	 * Function that return styles for header logo image
	 */
	function overworld_edge_logo_image_style( $style ) {
		$page_id              = overworld_edge_get_page_id();
		$class_prefix         = overworld_edge_get_unique_page_class( $page_id, true );
		$logo_image_selector  = array(
			$class_prefix . ' .edgtf-page-header .edgtf-menu-area .edgtf-logo-wrapper a img',
			$class_prefix . ' .edgtf-vertical-menu-area .edgtf-logo-wrapper a img'
		);

		$current_style = '';

		$logo_image_style      = array();
		$logo_box_shadow       = overworld_edge_get_meta_field_intersect( 'logo_enable_box_shadow', $page_id );

		if($logo_box_shadow === 'yes') {
			$logo_image_style['box-shadow'] = '0px 10px 27px 0px rgba(255,0,0, 0.5);';
		}

		if ( ! empty( $logo_image_style ) ) {
			$current_style .= overworld_edge_dynamic_css( $logo_image_selector, $logo_image_style );
		}
	...
	add_filter( 'overworld_edge_filter_add_page_custom_style', 'overworld_edge_logo_image_style' );
}