 ...
 */
if ( ! class_exists( 'OverworldEdgeClassSidebar' ) ) {
	class OverworldEdgeClassSidebar {
		var $sidebars = array();
		var $stored = "";
		...
		function __construct() {
			...
			$this->title  = esc_html__( 'Custom Widget Area', 'overworld' );
		...
		function load_assets() {
			...
			wp_enqueue_script( 'overworld-edge-admin-sidebar', OVERWORLD_EDGE_ROOT . '/framework/admin/assets/js/edgtf-ui/edgtf-sidebar.js' );
			wp_enqueue_style( 'overworld-edge-admin-sidebar', OVERWORLD_EDGE_ROOT . '/framework/admin/assets/css/edgtf-sidebar.css' );

			wp_localize_script(
				'overworld-edge-admin-sidebar',
				'edge',
				array(
		...
		function template_add_widget_field() {
			...
			echo "\n    <span class='input_wrap'><input type='text' value='' placeholder = '" . esc_attr__('Enter Name of the new Widget Area', 'overworld') . "' name='edgtf-add-widget' /></span>";
			echo "\n    <input class='button' type='submit' value='" . esc_attr__('Add Widget Area', 'overworld') . "' />";
			echo "\n    " . $nonce;
			echo "\n  </form>";
			echo "\n</script>\n";
		}
		...
		function delete_sidebar_area() {
			...
			if ( ! empty( $_POST['name'] ) ) {
				...
				if ( ( $key = array_search( $name, $this->sidebars ) ) !== false ) {
					...
					update_option( $this->stored, $this->sidebars );
					echo esc_html__( "sidebar-deleted", 'overworld' );
				}
			}
		...
		function register_custom_sidebars() {
			...
			$args = apply_filters( 'overworld_edge_filter_custom_widget_args', $args );
