...
if ( class_exists( 'OverworldCoreClassWidget' ) ) {
	class OverworldEdgeClassClassIconsGroupWidget extends OverworldCoreClassWidget {
		public function __construct() {
			parent::__construct(
				'edgtf_social_icons_group_widget',
				esc_html__( 'Overworld Social Icons Group Widget', 'overworld' ),
				array( 'description' => esc_html__( 'Use this widget to add a group of up to 6 social icons to a widget area.', 'overworld' ) )
			);
			...
			$this->params = array_merge(
				...
				array(
					array(
						'type'  => 'textfield',
						'name'  => 'label',
						'title' => esc_html__( 'Label', 'overworld' )
					)
				),
				overworld_edge_icon_collections()->getSocialIconWidgetMultipleParamsArray( 6 ),
				array(
					array(
						'type'    => 'dropdown',
						'name'    => 'layout',
						'title'   => esc_html__( 'Icons Layout', 'overworld' ),
						'options' => array(
							''             => esc_html__( 'Default', 'overworld' ),
							'square-icons' => esc_html__( 'Square', 'overworld' ),
						)
					),
					array(
						'type'        => 'dropdown',
						'name'        => 'skin',
						'title'       => esc_html__( 'Square Icons Skin', 'overworld' ),
						'description' => esc_html__( 'This applies to the square layout', 'overworld' ),
						'options'     => array(
							''           => esc_html__( 'Dark Skin', 'overworld' ),
							'light-skin' => esc_html__( 'Light Skin', 'overworld' ),
						)
					),
					array(
						'type'    => 'dropdown',
						'name'    => 'alignment',
						'title'   => esc_html__( 'Text Alignment', 'overworld' ),
						'options' => array(
					...
					array(
						'type'  => 'textfield',
						'name'  => 'icon_size',
						'title' => esc_html__( 'Icons Size (px)', 'overworld' )
					),
					array(
						'type'  => 'colorpicker',
						'name'  => 'color',
		...
		public function widget( $args, $instance ) {
			...
			if ( ! empty( $instance['label'] ) ) {
				echo '<span class="edgtf-label">' . esc_html( $instance['label'] ) . '</span>';
			}
			...
			for ( $n = 1; $n <= 6; $n ++ ) {
				...
				<?php if ( ! empty( $icon_holder_html ) ) { ?>
					<a class="edgtf-social-icon-widget-holder edgtf-icon-has-hover" <?php echo overworld_edge_get_inline_attr( $hover_color, 'data-hover-color' ); ?> <?php overworld_edge_inline_style( $icon_styles ) ?> href="<?php echo esc_url( $link ); ?>" target="<?php echo esc_attr( $target ); ?>">
