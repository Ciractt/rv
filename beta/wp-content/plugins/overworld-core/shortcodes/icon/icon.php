<?php
namespace OverworldCore\CPT\Shortcodes\Icon;
...
class Icon implements Lib\ShortcodeInterface {
    ...
    public function vcMap() {
	    if(function_exists('vc_map')) {
		    vc_map(
		    	array(
				    'name'                      => esc_html__( 'Icon', 'overworld-core' ),
				    'base'                      => $this->base,
					    ...
					    array(
						    array(
							    ...
							    'value'      => array(
								    ...
								    esc_html__( 'Huge', 'overworld-core' )   => 'edgtf-icon-huge'
							    )
						    ),
						    array(
							    'type'       => 'textfield',
							    'param_name' => 'custom_size',
							    'heading'    => esc_html__( 'Custom Size (px)', 'overworld-core' )
						    ),
						    array(
							    'type'       => 'dropdown',
							    'param_name' => 'type',
						    ...
						    array(
							    ...
							    'heading'     => esc_html__( 'Border Radius', 'overworld-core' ),
							    'description' => esc_html__( 'Please insert border radius(Rounded corners) in px. For example: 4 ', 'overworld-core' ),
						    ...
						    array(
							    ...
							    'heading'    => esc_html__( 'Border Color', 'overworld-core' ),
							    'dependency' => array( 'element' => 'type', 'value' => array( 'edgtf-circle', 'edgtf-square' ) )
						    ),
						    array(
							    'type'       => 'textfield',
							    'param_name' => 'border_width',
							    'heading'    => esc_html__( 'Border Width', 'overworld-core' ),
							    'dependency' => array( 'element' => 'type', 'value'   => array( 'edgtf-circle', 'edgtf-square' ) )
						    ),
						    array(
							    'type'       => 'colorpicker',
						    ...
						    array(
							    ...
							    'heading'    => esc_html__( 'Hover Icon Color', 'overworld-core' )
						    ),
						    array(
							    'type'       => 'colorpicker',
							    'param_name' => 'hover_border_color',
							    'heading'    => esc_html__( 'Hover Border Color', 'overworld-core' ),
							    'dependency' => array( 'element' => 'type', 'value' => array( 'edgtf-circle', 'edgtf-square' ) )
						    ),
						    array(
							    'type'       => 'colorpicker',
						    ...
						    array(
							    ...
							    'heading'     => esc_html__( 'Use Link as Anchor', 'overworld-core' ),
							    'value'       => array( 'Use this icon as Anchor?' => 'yes' ),
							    'description' => esc_html__( 'Check this box to use icon as anchor link (eg. #contact)', 'overworld-core' ),
	...
	public function render( $atts, $content = null ) {
		...
		$html = overworld_core_get_shortcode_module_template_part( 'templates/icon', 'icon', '', $params );
	...
	private function generateIconStyles( $params ) {
		...
		if ( ( $params['type'] !== 'edgtf-normal' && ! empty( $params['shape_size'] ) ) || ( $params['type'] == 'edgtf-normal' ) ) {
			if ( ! empty( $params['custom_size'] ) ) {
				$iconStyles[] = 'font-size:' . overworld_edge_filter_px( $params['custom_size'] ) . 'px';
			}
		}
	...
	private function generateIconHolderStyles( $params ) {
		...
		if ( $params['type'] !== 'edgtf-normal' ) {
			...
			if ( ! empty( $shapeSize ) ) {
				$iconHolderStyles[] = 'width: ' . overworld_edge_filter_px( $shapeSize ) . 'px';
				$iconHolderStyles[] = 'height: ' . overworld_edge_filter_px( $shapeSize ) . 'px';
				$iconHolderStyles[] = 'line-height: ' . overworld_edge_filter_px( $shapeSize ) . 'px';
			}
			...
			if ( ! empty( $params['border_color'] ) && ( isset( $params['border_width'] ) && $params['border_width'] !== '' ) ) {
				...
				$iconHolderStyles[] = 'border-width: ' . overworld_edge_filter_px( $params['border_width'] ) . 'px';
			...
			if ( $params['type'] == 'edgtf-square' ) {
				if ( isset( $params['border_radius'] ) && $params['border_radius'] !== '' ) {
					$iconHolderStyles[] = 'border-radius: ' . overworld_edge_filter_px( $params['border_radius'] ) . 'px';
				}
			}
		}
