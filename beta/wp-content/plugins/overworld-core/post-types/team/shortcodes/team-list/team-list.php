...
class TeamList implements Lib\ShortcodeInterface {
    ...
    public function __construct() {
	    ...
	    add_filter( 'vc_autocomplete_edgtf_team_list_selected_teams_callback', array( &$this, 'teamIdAutocompleteSuggester', ), 10, 1 ); // Get suggestion(find). Must return an array

	    //Team selected projects render
	    add_filter( 'vc_autocomplete_edgtf_team_list_selected_teams_render', array( &$this, 'teamIdAutocompleteRender', ), 10, 1 ); // Render exact team. Must return an array (label,value)
    }
    ...
    public function vcMap() {
	    if(function_exists('vc_map')) {
		    vc_map(
		    	array(
				    'name'                      => esc_html__( 'Team List', 'overworld-core' ),
				    'base'                      => $this->getBase(),
	...
	public function render( $atts, $content = null ) {
		...
		$html = overworld_core_get_cpt_shortcode_module_template_part( 'team', 'team-list', 'team-holder', '', $params, $additional_params );
	...
	public function getQueryArray( $params ) {
		...
		$team_ids = null;
		if ( ! empty( $params['selected_teams'] ) ) {
			$team_ids              = explode( ',', $params['selected_teams'] );
            $query_array['orderby'] = 'post__in';
			$query_array['post__in'] = $team_ids;
		}
