...
class MatchList implements Lib\ShortcodeInterface {
    ...
    public function __construct() {
	    ...
	    add_filter( 'vc_autocomplete_edgtf_match_list_category_callback', array( &$this, 'matchCategoryAutocompleteSuggester', ), 10, 1 ); // Get suggestion(find). Must return an array

	    //Match category render
	    add_filter( 'vc_autocomplete_edgtf_match_list_category_render', array( &$this, 'matchCategoryAutocompleteRender', ), 10, 1 ); // Get suggestion(find). Must return an array

	    //Match selected projects filter
	    add_filter( 'vc_autocomplete_edgtf_match_list_selected_matches_callback', array( &$this, 'matchIdAutocompleteSuggester', ), 10, 1 ); // Get suggestion(find). Must return an array

	    //Match selected projects render
	    add_filter( 'vc_autocomplete_edgtf_match_list_selected_matches_render', array( &$this, 'matchIdAutocompleteRender', ), 10, 1 ); // Render exact match. Must return an array (label,value)
    }
    ...
    public function vcMap() {
	    if(function_exists('vc_map')) {
		    vc_map(
		    	array(
				    'name'                      => esc_html__( 'Match List', 'overworld-core' ),
				    'base'                      => $this->getBase(),
	...
	public function render( $atts, $content = null ) {
		...
		$html = overworld_core_get_cpt_shortcode_module_template_part( 'match', 'match-list', 'match-holder', '', $params, $additional_params );
