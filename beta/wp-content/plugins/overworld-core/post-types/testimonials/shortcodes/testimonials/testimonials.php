...
namespace OverworldCore\CPT\Shortcodes\Testimonials;
...
class Testimonials implements Lib\ShortcodeInterface {
				...
				array(
					...
					'params'                    => array(
						array(
							...
							'value'       => array(
								esc_html__( 'Standard', 'overworld-core' )            => 'standard',
								esc_html__( 'Boxed', 'overworld-core' )               => 'boxed',
								esc_html__( 'Image Pagination', 'overworld-core' )    => 'image-pagination',
								esc_html__( 'Boxed Text', 'overworld-core' )          => 'boxed-text',
								esc_html__( 'Carousel', 'overworld-core' )            => 'carousel'
							),
							'save_always' => true
						),
						array(
							...
							'value'       => array(
								...
								esc_html__( 'Light', 'overworld-core' )   => 'light',
							),
							'save_always' => true
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'number',
							'heading'    => esc_html__( 'Number of Testimonials', 'overworld-core' )
						),
						array(
							'type'        => 'autocomplete',
							'param_name'  => 'category',
							'heading'     => esc_html__( 'Category', 'overworld-core' ),
							'description' => esc_html__( 'Enter one category slug (leave empty for showing all categories)', 'overworld-core' )
						),
						array(
							'type'       => 'colorpicker',
							'param_name' => 'box_color',
							'heading'    => esc_html__( 'Content Box Color', 'overworld-core' ),
							'dependency' => Array( 'element' => 'type', 'value' => 'boxed' )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'number_of_visible_items',
							'heading'     => esc_html__( 'Number Of Visible Items', 'overworld-core' ),
							'value'       => array(
								esc_html__( 'One', 'overworld-core' )   => '1',
								esc_html__( 'Two', 'overworld-core' )   => '2',
								esc_html__( 'Three', 'overworld-core' ) => '3',
								esc_html__( 'Four', 'overworld-core' )  => '4',
								esc_html__( 'Five', 'overworld-core' )  => '5',
								esc_html__( 'Six', 'overworld-core' )   => '6'
							),
							'save_always' => true,
	...
	public function render( $atts, $content = null ) {
        ...
        return overworld_core_get_cpt_shortcode_module_template_part( 'testimonials', 'testimonials', 'testimonials-' . $params['type'], '', $params );
	...
	public function testimonialsCategoryAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				...
				$data['label'] = ( ( strlen( $value['testimonials_category_title'] ) > 0 ) ? esc_html__( 'Category', 'overworld-core' ) . ': ' . $value['testimonials_category_title'] : '' );
				$results[]     = $data;
			}
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( is_object( $testimonials_category ) ) {
				...
				if ( ! empty( $testimonials_category_title ) ) {
					$testimonials_category_title_display = esc_html__( 'Category', 'overworld-core' ) . ': ' . $testimonials_category_title;
				}
