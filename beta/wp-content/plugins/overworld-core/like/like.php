...
class OverworldEdgeClassLike {
	private static $instance;
	
	private function __construct() {
		add_action( 'wp_ajax_overworld_core_action_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_overworld_core_action_like', array( $this, 'ajax' ) );
	}
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'overworld-core' );
		
		if ( isset( $_COOKIE[ 'edgtf-like_' . $post->ID ] ) ) {
			$class = 'edgtf-like liked';
			$title = esc_html__( 'You already like this!', 'overworld-core' );
		}
...
if ( ! function_exists( 'overworld_core_create_like' ) ) {
	function overworld_core_create_like() {
		OverworldEdgeClassLike::get_instance();
	}
	
	add_action( 'after_setup_theme', 'overworld_core_create_like' );
}